...
test('CJS computePercentageExplained should match expected values for first', () => {
...
test('CJS computePercentageExplained should match expected values for top two', () => {
  const vectors = PCA.getEigenVectors(testData);
  const topTwo = PCA.computePercentageExplained(vectors, vectors[0], vectors[1]);
  expect(topTwo).toBeCloseTo(0.9700602484397556);
});

test('CJS getEigenVectors should return expected eigenvalues and vectors', () => {
  const vectors = PCA.getEigenVectors(testData);
...
test('CJS computeAdjustedData should return expected structure', () => {
  const vectors = PCA.getEigenVectors(testData);
